...
| بخش | مشکل | راه‌حل |
| --- | --- | --- |
| پوشه‌ی [`extension/`](extension/) | چت `claude.ai` در مرورگر چپ‌چین است | افزونه‌ی مرورگر / یوزراسکریپت |
| پوشه‌ی [`claude-code/`](claude-code/) | متن فارسی در ترمینال Claude Code به‌هم می‌ریزد | اسکیل + قانون نگارشی |
...
- صفت `dir="auto"` متنی را که داخل یک فرزندِ دارای صفت `dir` باشد نادیده می‌گیرد؛ در نتیجه یک `<ul>` که `<li>`هایش قبلاً علامت‌گذاری شده‌اند همیشه LTR می‌ماند.
- صفت `dir="auto"` فقط به **اولین** کاراکتر جهت‌دار نگاه می‌کند؛ جمله‌ی فارسی‌ای که با یک نام انگلیسی یا `inline code` شروع شود اشتباه چپ‌چین می‌شد.

چون به هیچ نام کلاسی از خود claude.ai وابسته نیست، با تغییر ظاهر سایت هم نمی‌شکند.

علاوه بر آن:

- مقادیر `padding` و `border` سمت چپِ لیست‌ها و نقل‌قول‌ها در حالت RTL آینه می‌شود (Tailwind از مقادیر فیزیکی استفاده می‌کند).
- تگ‌های `pre`، `code`، `kbd` و `.katex` همیشه LTR می‌مانند.
...
آیکون‌ها در `extension/icons/` هستند و با یک اسکریپت ساخته می‌شوند، نه دستی:

```bash
python tools/make_icons.py
```

نشانه‌اش سه نوار راست‌چین است — یعنی همان کاری که افزونه می‌کند. همه چیز در چهار برابر اندازه کشیده و بعد کوچک می‌شود تا لبه‌های آیکون ۱۶ پیکسلی تمیز بماند. اسکریپت به Pillow نیاز دارد.

## استفاده

- کلید **Ctrl + Alt + R** — روشن/خاموش کردن RTL (وضعیت در `localStorage` ذخیره می‌شود).
...
| کلید | پیش‌فرض | توضیح |
| --- | --- | --- |
| `rtlThreshold` | `0.25` | حداقل نسبت کاراکتر RTL برای راست‌چین شدن یک بلوک <!-- rtl-ok --> |
| `sidebar` | `true` | عنوان گفتگوها در سایدبار هم جهت می‌گیرد؛ اگر چیدمان به‌هم ریخت خاموشش کن <!-- rtl-ok --> |
| `persianFont` | `true` | اگر `false` شود، فونت خود claude.ai دست‌نخورده می‌ماند <!-- rtl-ok --> |
| `fontStack` | Vazirmatn → Tahoma | ترتیب فونت‌های فارسی <!-- rtl-ok --> |
| `hotkey` | `Ctrl+Alt+R` | میان‌بر روشن/خاموش <!-- rtl-ok --> |
| `defaultEnabled` | `true` | وضعیت اولیه در اولین اجرا <!-- rtl-ok --> |

## تست بدون claude.ai

فایل [`test/rtl-sandbox.html`](test/rtl-sandbox.html) یک صفحه‌ی نمونه با ساختار مشابه چت کلاد است: پیام‌های دوزبانه، لیست، نقل‌قول، بلوک کد، سایدبار، و یک دکمه که استریم شدن پاسخ را شبیه‌سازی می‌کند. مستقیم در مرورگر بازش کنید تا رفتار اسکریپت را ببینید.

همان صفحه، تست خودکار هم دارد:

```bash
node test/run-browser-tests.mjs
```

بیست‌وسه ادعا در کروم هدلس اجرا می‌شود. نه چارچوب تستی لازم دارد نه دانلود مرورگر: صفحه خودش ادعاها را اجرا می‌کند و نتیجه را داخل DOM می‌نویسد، و `chrome --dump-dom` بیرونش می‌آورد. اگر کروم جای غیرمعمولی نصب است، مسیرش را در متغیر `CHROME` بگذارید.

اعتبار خود سوئیت با سه جهش سنجیده شد — خاموش کردن سایدبار، حذف آینه‌کردن `padding`، و حذف fallback زمان‌سنج — و هر سه بار تست‌های مربوطه قرمز شدند.
